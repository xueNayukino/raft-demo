...
func (s *Snapshotter) countLogEntries() (uint64, error) {
	...
	err := s.db.View(func(txn *badger.Txn) error {
		...
		for it.Seek(prefixLogEntry); it.Valid(); it.Next() { //将迭代器定位到 第一个匹配 prefixLogEntry
			key := it.Item().Key()
...
func (s *Snapshotter) checkAndCreateSnapshot() error {
	...
	snapshot := raftpb.Snapshot{
		...
		Metadata: raftpb.SnapshotMetadata{
			Index:     committedIndex, //提交的索引，快照点
			Term:      s.getCurrentTerm(),
			ConfState: confState, //当前配置状态
		},
	}
	...
	if err := s.db.Update(func(txn *badger.Txn) error {
		...
		deleted := 0
		for it.Seek(prefixLogEntry); it.Valid(); it.Next() {
			key := it.Item().Key() //获取当前键，实质是一个byte切片
			if !bytes.HasPrefix(key, prefixLogEntry) {
				break
			}
			index := binary.BigEndian.Uint64(key[len(prefixLogEntry):]) //从key中提取索引，开始的第一条日志的索引
	...
	fmt.Printf("快照创建完成，重置日志条目计数\n") //快照触发逻辑是基于“新日志条目数”而不是“所有未被快照日志条目数”
...
func (s *Snapshotter) SaveSnapshot(snapshot raftpb.Snapshot) error {
	...
	//使用 filepath.Join 构造临时文件路径，文件名为 tmp-<index>，其中 <index> 是快照的索引（snapshot.Metadata.Index），格式化为 16 位十六进制（%016x 保证补零）
	tmpPath := filepath.Join(s.dir, fmt.Sprintf("tmp-%016x", snapshot.Metadata.Index))
	fmt.Printf("[SaveSnapshot] 创建临时快照文件: %s\n", tmpPath)
	//os.O_WRONLY：以只写模式打开。os.O_CREATE：如果文件不存在则创建。os.O_TRUNC：如果文件已存在，清空内容。0600：文件权限为用户读写，其他用户无权限
	...
	if _, err := fp.Write(data); err != nil {
		os.Remove(tmpPath) // 清理临时文件，如果写入失败，调用 os.Remove(tmpPath) 删除临时文件（避免留下不完整数据）
		fmt.Printf("[SaveSnapshot] 写入快照数据失败: %v\n", err)
		return fmt.Errorf("写入快照数据失败: %v", err)
	}

	// 确保数据写入磁盘，调用 fp.Sync() 确保数据从内存缓冲区写入磁盘
	...
	// 构建最终文件名，eg:0000000000000002-000000000000000A.snap
	...
	// 重命名临时文件，原子操作，确保快照文件要么完整存在，要么不存在
...
func (s *Snapshotter) LoadSnapshot() (*raftpb.Snapshot, error) {
	...
	filename := names[len(names)-1] // 最新的快照，相对目录
...
// ListSnapshots 列出所有快照文件，里面进行了排序操作，第一个为最旧的快照
func (s *Snapshotter) ListSnapshots() ([]string, error) {
	fmt.Printf("[ListSnapshots] 进入函数体: %s\n", s.dir)
	// 不再加锁，避免死锁

	dir, err := os.Open(s.dir) //打开快照目录，返回一个目录句柄 dir
	if err != nil {
	...
	names, err := dir.Readdirnames(-1) //读取目录下所有文件和子目录的名字，返回字符串切片 names
	if err != nil {
	...
	for _, name := range names {
		if strings.HasSuffix(name, snapshotSuffix) { //二次验证键前缀（防御性编程），后是前的一个子集
			snaps = append(snaps, name)
		}
	}

	sort.Strings(snaps) //对快照文件名进行排序,方便以后找到最新快照
