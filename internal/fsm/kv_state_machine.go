...
type Transaction struct {
	ID       uint64    `json:"id"`       // 事务ID，唯一标识符
	State    string    `json:"state"`    // 事务状态
	Index    uint64    `json:"index"`    // 与事务关联的Raft日志索引
	Commands []Command `json:"commands"` // 事务包含的命令
}

// ReadIndexContext 表示一致性读的上下文
type ReadIndexContext struct {
	RequiredIndex uint64        // 要求状态机至少应用到的 Raft 日志索引
	ReadResult    chan struct{} // 读取完成信号
}

// SnapshotData 表示状态机快照 数据
type SnapshotData struct {
	Data         map[string]string `json:"data"`
	AppliedIndex uint64            `json:"applied_index"` //快照时的已应用的最大 Raft 日志索引
}

// KVStateMachine 是一个基于 BadgerDB 的键值状态机
type KVStateMachine struct {
	db           *badger.DB
	mu           sync.RWMutex            //读写锁，保护状态机的并发访问
	appliedIndex uint64                  // 已应用的最大Raft日志索引
	txns         map[uint64]*Transaction // 活跃事务表，键为事务 ID，值为事务指针
	txnMu        sync.RWMutex
	readIndexes  sync.Map   // 跟踪ReadIndex请求，同步映射，跟踪一致性读请求（键为索引，值为通知通道）
	indexCond    *sync.Cond //条件变量，用于等待 Raft 日志索引更新
}
...
func (sm *KVStateMachine) Apply(entry raftpb.Entry) error {
	...
	if entry.Type == raftpb.EntryConfChange {
		...
		if entry.Index > sm.appliedIndex {
			...
			sm.indexCond.Broadcast() // 通知等待的条件变量，广播条件变量，通知所有等待索引更新的 goroutine
		}
		sm.mu.Unlock()
		return nil
	}
...
func (kv *KVStateMachine) WaitForIndex(index uint64) error {
	//如果已经应用到索引，立即返回
	if atomic.LoadUint64(&kv.appliedIndex) >= index { //可以设计一个多少数据内成功返回
		return nil
	}

	currentIndex := atomic.LoadUint64(&kv.appliedIndex)
	if index-currentIndex <= 5 {
		//	fmt.Printf("索引差距较小(%d vs %d)，允许立即读取\n", currentIndex, index)
		return nil
	}
...
// notifyReadIndex 通知等待指定索引的ReadIndex请求，index uint64：当前状态机已应用的日志索引（appliedIndex）
	...
	kv.readIndexes.Range(func(key, value interface{}) bool {
		if waitIndex := key.(uint64); waitIndex <= index {
			if ch, ok := value.(chan struct{}); ok {
				close(ch) //被 close(ch) 唤醒，满足才会发这个通知
			}
			kv.readIndexes.Delete(key) //删除已通知的请求
		}
		return true
	})
}
...
func (kv *KVStateMachine) Delete(key string) error {
	...
	// 开始事务，手动事物不用update,方便执行更复杂的操作
	txn := kv.db.NewTransaction(true) //创建 Badger 数据库的写事务（txn），参数 true 表示这是一个可写事务（而非只读事务）
	defer txn.Discard()               //确保在函数返回时，事务被丢弃（回滚），避免资源泄漏，如果事务已通过 txn.Commit() 提交，Discard 不会有任何效果
