...
type Transport struct {
	...
	// gRPC 客户端连接池
	conns   map[uint64]*grpc.ClientConn //grpc.ClientConn表示到其他节点的持久连接
	connsMu sync.RWMutex

	// 消息处理器
	handler MessageHandler //handler 处理接收到的 Raft 消息，接收消息的回调处理器接口

	// 上下文控制
	ctx    context.Context
	cancel context.CancelFunc //取消函数，关闭传输层

	// 流式消息管理
	streams   map[uint64]RaftService_StreamMessageClient //到各节点的双向gRPC流（节点ID → 流对象）
	streamMu  sync.RWMutex
	reconnect chan uint64 // 重连通道，传递需要重连的节点ID
	sequence  uint64      // 消息序列号，用于消息排序/去重）

	// 快照传输管理
	snapshots   map[uint64]*snapshotState //正在传输的快照状态（节点ID → 传输状态）
	snapshotMu  sync.RWMutex
	chunkSize   int
	maxChunkNum uint64 //最大并行传输的快照块数量

	// 离线节点管理
	offlineNodes      map[uint64]time.Time // 离线节点记录（节点ID → 离线时间戳
	offlineNodesMu    sync.RWMutex         //对应的Mu锁保护各自映射
	failureCounter    map[uint64]int       //连接失败计数器（节点ID → 连续失败次数）
	failureCounterMu  sync.RWMutex
	offlineTimeout    time.Duration // 节点被标记为离线的超时时间。节点离线判定超时
	reconnectInterval time.Duration // 重连间隔

	UnimplementedRaftServiceServer //保证服务向前兼容
}
...
type snapshotState struct {
	...
	chunkSize   int
	currentSize int
	chunks      map[uint64][]byte //块索引和块数据
	metadata    *SnapshotMetadata
	done        chan struct{} //通知快照传输完成的通道
	fromNode    uint64        // 添加源节点ID字段，追踪快照是从哪个节点接收的
}

// MessageHandler 定义了消息处理接口，解耦传输层与状态机逻辑
type MessageHandler interface {
	HandleMessage(msg raftpb.Message) error //获取消息
	GetTerm() uint64
	GetLeaderID() uint64
	ProposeConfChange(ctx context.Context, cc raftpb.ConfChange) error //cc raftpb.ConfChange：配置变更请求（添加/移除节点等）
}
...
// GetPeerAddrs 返回所有节点ID到地址的映射，获取集群中所有节点的地址信息，包括自己和对等节点，常用于集群状态查询和配置更新
...
func (t *Transport) dialPeer(id uint64, addr string) error {
	...
	opts := []grpc.DialOption{
		grpc.WithTransportCredentials(insecure.NewCredentials()), //不使用 TLS 加密
	...
	conn, err := grpc.DialContext(ctx, addr, opts...) //创建到指定节点的gRPC连接，DialContext 是 gRPC 的异步连接方法，实际 TCP 连接在首次 RPC 调用时建立（懒连接）
	if err != nil {
	...
	if oldConn, ok := t.conns[id]; ok {
		oldConn.Close() //关闭旧连接（避免泄漏），存储新连接
	}
	t.conns[id] = conn //将新连接添加到连接池中
	t.connsMu.Unlock()

	// 建立流式连接
	client := NewRaftServiceClient(conn)
	stream, err := client.StreamMessage(t.ctx) //StreamMessage 发起双向流式 RPC（对应 proto 定义的 rpc StreamMessage(stream Request) returns (stream Response)
	if err != nil {
	...
	t.streams[id] = stream //将新流添加到 streams 映射中
	t.streamMu.Unlock()

	return nil
}

// 这是 gRPC 自动生成的一个客户端流接口，用于 RaftService 的 StreamMessage 方法。它实现了发送（Send）、接收（Recv）、关闭（CloseSend）等方法，它代表了节点间的一个双向流式连接，你可以在这个连接上不断地发送和接收消息，而不用每次都新建连接
func (t *Transport) getStream(nodeID uint64) (RaftService_StreamMessageClient, bool) {
	t.streamMu.RLock()
	defer t.streamMu.RUnlock()
	stream, ok := t.streams[nodeID] // 3. 获取到某个节点（nodeID）对应的 gRPC 流式连接（StreamMessage 流）
	return stream, ok
}

// 通过已建立的流发送消息并等待响应
func (t *Transport) sendStream(stream RaftService_StreamMessageClient, req *RaftMessageRequest) error {
	if err := stream.Send(req); err != nil { //通过流发送消息
...
func (t *Transport) sendRPC(nodeID uint64, req *RaftMessageRequest) error {
	...
	// 检查连接状态
	state := conn.GetState()
	if state == connectivity.Shutdown || state == connectivity.TransientFailure { //conn.GetState()：获取 gRPC 连接的当前状态，检查状态是否为 Shutdown（已关闭）或 TransientFailure（暂时失败）
		// 尝试重新连接
		t.peersMu.RLock()
	...
	client := NewRaftServiceClient(conn)
	resp, err := client.SendMessage(ctx, req) //调用单向 RPC 方法发送请求
