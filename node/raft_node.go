...
type RaftNode struct {
	...
	// 通道
	stopc               chan struct{}
	confChangeCompleted chan struct{}
	proposeCh           chan []byte
	confChangeCh        chan raftpb.ConfChange
	readIndexCh         chan *readIndexRequest

	// 读索引映射
	readIndexes map[string]*readIndexContext
...
func NewRaftNode(config RaftNodeConfig, fsm *fsm.KVStateMachine) (*RaftNode, error) {
	...
	if config.ElectionTick <= 0 {
		config.ElectionTick = 10 // 默认值
	}
	if config.HeartbeatTick <= 0 {
		config.HeartbeatTick = 1 // 默认值
	}
	if config.SnapshotCount == 0 {
		config.SnapshotCount = 10000 // 默认值
	}
	if config.MaxSizePerMsg == 0 {
		config.MaxSizePerMsg = 1024 * 1024 // 默认值 1MB
	}
	if config.MaxInflightMsgs == 0 {
		config.MaxInflightMsgs = 256 // 默认值
	}
	if config.CheckpointInterval <= 0 {
		config.CheckpointInterval = 30 // 默认值 30秒
	}
	...
	// 创建网络层
	rn.transport, err = transport.NewTransport(config.NodeID, config.ListenAddr, rn)
	if err != nil {
		cancel()
		rn.storage.Stop()
	...
	c := &raft.Config{
		...
		PreVote:         true, // 启用 PreVote 以防止网络分区导致的不必要选举
	}

	// 如果是单节点集群，直接成为领导者
	if len(config.Peers) == 1 && !config.IsJoin {
		c.Applied = 0
		c.DisableProposalForwarding = true // 禁用提议转发
		// 确保这个节点能立即成为领导者
